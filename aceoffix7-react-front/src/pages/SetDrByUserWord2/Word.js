...
const Word = () => {
  ...
  const Save = () => {
    aceoffixctrl.SaveDataPage = `/SetDrByUserWord2/saveData?userName=`+aceoffixctrl.WindowParams;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await service.post(`/SetDrByUserWord2/Word?userName=`+aceoffixctrl.WindowParams);
        if (response) {
