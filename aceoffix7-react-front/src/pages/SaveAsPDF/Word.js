...
const Word = () => {
  ...
  const OnAceoffixCtrlInit = () => {
    ...
    aceoffixctrl.AddCustomToolButton("Save as PDF", "SaveAsPDF", 1);
  };

  const SaveAsPDF = () => {
    aceoffixctrl.SaveFilePage = "/SaveAsPDF/save";
    aceoffixctrl.WebSaveAsPDF();
    alert("The PDF file has been saved to the server.");
  };
  ...
  const openFile = async () => {
    try {
      const response = await service.post("/SaveAsPDF/Word");
      return response;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    window.ACEPageMounted = { OnAceoffixCtrlInit, AfterDocumentOpened, SaveAsPDF,Save };
